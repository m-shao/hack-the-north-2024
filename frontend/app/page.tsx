...
const Page = () => {
  const [data, setData] = useState<string | null>(""); // Initialize with null or empty object
  const [lastModified, setLastModified] = useState(null);
  const [error, setError] = useState(null);

  async function updateFirestoreWithNearme(
    nearMe: String[]
  ){
    try {
      ...
      console.log(docRef)
      console.log("Updating: ", nearMe);


      // Update the "front to back" field in the document
      const res = await updateDoc(docRef,{"near me" : nearMe.join(",")});
  ...
  async function updateFirestoreWithInstructions(
    ...
    try {
      ...
      const res = await updateDoc(docRef, {
        "front to back": `${instruction.action.type} ${
          instruction.action.bearing ?? ""
        } and walk ${Math.round(instruction.distance / 0.7)} steps`, // Adjust as per your instruction structure
      });
    ...
    const checkFirestoreDoc = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (
            jsonData["back to front"] !== "" &&
            jsonData["back to front"] !== data
          ) {
            setData(jsonData["back to front"]);
          }
        }
      } catch (err) {
        setError(err);
  ...
  async function init(data) {
    ...
    endPosition =spaces.indexOf(spaces.find((item) => item.name === data));
    ...
    function updateAsset(a: Asset, duration: number) {
      if (!isPause.current) {
        ...
        if (foundInstruction) {
          updateFirestoreWithInstructions(instruction);
          findNearbyPOIs()
          setTranscription(
  ...
  async function findNearbyPOIs() {
    if (currentCoordinates) {
      ...
      await updateFirestoreWithNearme(closestPOIs)
    } else {
    ...
    <>
      ...
      <div className="fixed bottom-4 left-4">
        ...
        <div className="flex row">
          <div className="mt-4 p-2 bg-white rounded-xl">
            <h1>
              <strong>Destinaton Address:</strong>
            </h1>
            <pre>{data}</pre>
          </div>
